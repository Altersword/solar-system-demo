...
class SolarSystem {
    ...
    createFocusedNeutronStarModel(entry) {
        ...
        const innerGlow = this.createGlowMesh(coreRadius * 1.35, 0xc8f4ff, 0.72);
        innerGlow.name = 'neutron-star-inner-glow';
        core.add(innerGlow);
        const corona = this.createHaloSprite(0x78d9ff, coreRadius * 5.4, 0.22);
        ...
        [-1, 1].forEach((direction) => {
            ...
            const hotspot = this.createHaloSprite(0xe6fbff, hotspotSize, 0.62);
    ...
    createPulsarBeamPair(coreRadius) {
        ...
        [-1, 1].forEach((direction) => {
            ...
            const random = this.seededRandom(`pulsar-beam-particles-${direction}`);
            const positions = [];
            ...
            const material = new THREE.PointsMaterial({
                color: 0xdaf8ff,
                size: 1.2,
                map: this.createRadialTexture(0xffffff),
                transparent: true,
                opacity: 0.28,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
    ...
    createPulsarWindNebula(entry, coreRadius) {
        const random = this.seededRandom(`${entry.id}-pulsar-wind`);
        const positions = [];
        const colors = [];
    ...
    createFocusedSpecialModel(entry) {
        if (entry.effectType === 'pulsar') {
            return this.createFocusedNeutronStarModel(entry);
        }
    ...
    clearFocusView() {
        if (!this.focusGroup && !this.focusRenderer) return;
        ...
        if (closeExpanded) closeExpanded.textContent = 'è¿”å›žæ˜Ÿå›¾';
        this.controls.minDistance = 18;
        this.controls.maxDistance = this.getModeConfig().maxDistance;
    }
