...
**星图阶段 C 与后续克制扩充已完成**：`CELESTIAL_CATALOG` 现为 **116** 个目标
（邻域 34 / 银河 46 / 本星系群 19 / 宇宙近邻 17），并按图层提供尺度距离壳。

**最小修复轮已完成**：
- Loading 在场景初始化后可靠关闭，失败时显示错误而不是永久遮罩。
- 近景观测按天体类型自动切换时间倍率，退出后恢复。
- 标签按距离优先、选中强制显示，并限制非选中标签数量。

**红巨星/白矮星增强已完成**：
- 独立 `RedGiantRenderer` / `WhiteDwarfRenderer`
- 新增毕宿五、大角星、米拉、范马南星、波江座 40 B

**太阳近景已完成**：
- 独立 `SunRenderer`：远看轻量，近景高细节
- 光球 / 色球 / 日冕 / 日珥分层

**太阳系近景阶段 1 已完成**：
- 所有行星、矮行星、已定义卫星、哈雷彗星均可近景观测
- 统一 `SolarSystemFocusRenderer` + 分类 `effectType`
- 地球/木星/土星/哈雷等已有类型级差异

**下一优先**：
1. 阶段 2：地球、木星、土星、火星、月球、哈雷个性化加强
2. 可选：太阳日冕纤维与黑子对比度继续微调
3. 可选：相机飞入曲线与信息面板布局打磨
4. 不建议：重写黑洞主路径、再冲超大规模目录
