...
**星图阶段 C 与后续克制扩充已完成**：`CELESTIAL_CATALOG` 现为 **50000** 个目标
（邻域 12505 / 银河 12518 / 本星系群 12490 / 宇宙近邻 12487），并按图层提供尺度距离壳。

**第 5 轮克制扩充已完成**：
- 太阳邻域：YZ 天坛座、L 98-59 两个可展开的多行星红矮星系统。
- 银河地标：NGC 7000 北美星云、M22 人马座球状星团。
- 本星系群：狮子座 A、SagDIG 两个孤立/边缘矮不规则星系。
- 宇宙近邻：M83 南风车星系、M94 猎犬座星系，补强正面旋臂与环状盘星系。

**200 项目录里程碑已完成**：
- 以 9 个小批次逐步加入 68 个目标；每批完成后均进行目录结构、必填字段和重复 ID 检查。
- 邻域补充近邻行星系统；银河补充星云、疏散/球状星团及超新星遗迹。
- 本星系群补充卫星和孤立矮星系；宇宙近邻补充旋涡、棒旋、侧向和活动核星系。

**300 项目录里程碑已完成**：
- 延续小批量校验方式：5 个批次、每批 20 项，四层各增加 5 项。
- 太阳邻域补入卢曼 16、WISE 0855−0714、51 Pegasi 等近邻褐矮星、双星与行星宿主。
- 银河地标补入火焰、气泡、新月等星云，以及 M4、M10、M92 等星团。
- 本星系群补入船底座、牧夫座、赛格与仙女座等超微弱或卫星矮星系。
- 宇宙近邻补入 M74、M77、NGC 1365、触角星系、NGC 4565 等不同形态星系。

**500 项目录里程碑已完成**：
- 继续采用 10 个小批次、每批 20 项的校验节奏，四层每批各增加 5 项。
- 太阳邻域增加 WISE/2MASS 褐矮星及格利泽红矮星样本；银河地标增加火焰之星、郁金香、猫爪等星云以及 M9/M14/M53/M54/M80 等星团。
- 本星系群补入 Antlia II、Tucana II–V、Virgo I/II 与更多仙女座卫星；宇宙近邻补入 M60–M61、M84–M91、M102、M105、M109 及多种 NGC 星系。

**1000 项目录里程碑已完成**：
- 延续小批量校验方式：25 个批次、每批 20 项，四层每批各增加 5 项。
- 太阳邻域补入 LHS 近邻红矮星序列；银河地标补入 Sharpless 电离氢区；本星系群补入 M31 球状星团编号序列；宇宙近邻补入 NGC 星系编号序列。
- 最终分层为邻域 255、银河 268、本星系群 240、宇宙近邻 237，保持四层并行扩充与目录校验覆盖。

**2000 项目录压力测试已完成**：
- 以 50 个小批次增加 1000 项，四层各增加 250 项；使用 LHS 1376–1625、Sharpless 2-127–376、M31 球状星团 B126–B375 与 NGC 5725–5974 编号序列。
- 最终分层为邻域 505、银河 518、本星系群 490、宇宙近邻 487；新增项目全部走批量点渲染。

**5000 项目录压力测试已完成**：
- 以 150 个小批次增加 3000 项，四层各增加 750 项；使用 LHS 1626–2375、WISE 电离氢区 W0001–W0750、M31 候选星团 C0001–C0750 与 UGC 10001–10750 索引。
- 最终分层为邻域 1255、银河 1268、本星系群 1240、宇宙近邻 1237；新增项目全部走批量点渲染。

**10000 项目录压力测试已完成**：
- 以 250 个小批次增加 5000 项，四层各增加 1250 项；使用 LHS 2376–3625、WISE 电离氢区 W0751–W2000、M31 候选星团 C0751–C2000 与 UGC 10751–12000 索引。
- 最终分层为邻域 2505、银河 2518、本星系群 2490、宇宙近邻 2487；新增项目全部走批量点渲染。

**20000 项目录压力测试已完成**：
- 以 500 个小批次增加 10000 项，四层各增加 2500 项；使用 LHS 3626–6125、WISE 电离氢区 W2001–W4500、M31 候选星团 C2001–C4500 与 UGC 12001–14500 索引。
- 最终分层为邻域 5005、银河 5018、本星系群 4990、宇宙近邻 4987；新增项目全部走批量点渲染。

**30000 项目录压力测试已完成**：
- 以 500 个小批次增加 10000 项，四层各增加 2500 项；使用 LHS 6126–8625、WISE 电离氢区 W4501–W7000、M31 候选星团 C4501–C7000 与 UGC 14501–17000 索引。
- 最终分层为邻域 7505、银河 7518、本星系群 7490、宇宙近邻 7487；新增项目全部走批量点渲染。

**50000 项目录压力测试已完成**：
- 以 1000 个小批次增加 20000 项，四层各增加 5000 项；使用 LHS 8626–13625、WISE 电离氢区 W7001–W12000、M31 候选星团 C7001–C12000 与 UGC 17001–22000 索引。
- 最终分层为邻域 12505、银河 12518、本星系群 12490、宇宙近邻 12487；新增项目全部走批量点渲染。

**五万级目录渲染基础已完成**：
- 已有 49500 个扩容目标标记为批量渲染层；四个星图图层各用一个 THREE.Points 批次承载 12375 项，启动时不再为它们逐项创建球体、光晕和标签。
- 搜索或点击批量星点时，目标会按需临时提升为高细节对象；切换到其他目标或图层后会回收该临时对象。
- 重点手工目录仍保留原有网格、标签、特效、信息面板与展开系统交互；后续扩容应默认走批量层。

## P0-P3 改进完成 · 2026-07-19

本批次完成四大方向改进：

### P0 · 卫星近景个性化
- Io: 新增表面熔岩辉光 + 4个独立相位火山热点/羽流
- Europa: lineae 从3条扩展到8条（5主弧+3混沌交叉）
- Titan: 新增南北极霾盖 + 18个烃湖暗斑
- 动画按天体类型分化（Io快节奏/Titan慢呼吸/Europa微光）

### P1 · 特殊天体视觉分化
- 脉冲星: 射束收窄3倍、核心闪白、峰值闪光源
- 超新星遗迹: 支持 shell / plerion / mixed 三种形态
- 红巨星: 大气层扩大(5.2x)、脉动加深、粒子风增多
- 白矮星: 核心更小(0.38x)、辉光更亮(0.62)、脉冲更快

### P2 · 相机叙事与信息面板
- 飞入曲线自适应时长(0.8-2.8s) + 末端超调
- 近景距离差异化（红巨星340/白矮星170/超新星380）
- 近景时底部显示时间倍率提示
- clear() 提取 _forceCleanup() 统一清理状态机

### P3 · 工程化与性能基线
- NEXT_STEPS.md 新建性能基线表（FPS/内存/帧耗时）
- 标签视锥裁剪优化（Frustum + 距离预筛）
- 全部19个JS文件通过 node --check 语法验证
**五万级目录继续复用首轮优化**：
- 49500 个批量条目统一声明为模拟压力测试数据，并在搜索结果与详情面板中明确提示。
- 批量星点方向使用黄金角低差异分布；严格校验 49500 个批量方向无重复，同时保留名称、坐标范围和数据来源校验。
- 搜索改为预建小写索引、最多返回 8 项并增加输入防抖；标签更新约 25 次/秒，尺寸只在创建时测量。
...
**近景观测类型全覆盖轮已完成**：
- `SpecialBodyFactory.getFocusType`：无 `effectType` 的精选目标按 category/type 推导近景类型；精选 500 项中 492 项可近景观察（此前 75 项），仅 8 个纯方向标除外。
- 新增 4 个专用渲染器：
  - `GalaxyRenderer`：旋涡/棒旋/椭圆/不规则/侧向星系（粒子盘 + 旋臂 + 尘埃带 + HII 结点），星系团模式为中央椭圆星系 + 9 个随机形态成员 + 星系团介质。
  - `NebulaRenderer`：发射星云（多瓣云团 + 暗尘埃带 + 内嵌年轻星团）与行星状星云（白矮星核 + 三层膨胀壳 + 赤道环）。
  - `StarClusterRenderer`：球状星团（9000 点 King 分布 + 红巨星/蓝离散星点缀）与疏散星团（松散热星 + 残余气体）。
  - `GenericStarRenderer`：主序星/巨星/超巨星/造父变星/WR 星按光谱分类外观；从类型文本识别双星/三合/四合/六合并渲染轨道伴星；红矮星并入并保留耀斑。
- `FocusController` 按新类型配置 bloom、相机距离与自转；近景时间倍率补充 7 类（星系 500、星系团 1000、星云 200、行星状星云 50、球状 300、疏散 100、普通恒星 5）。
- 观测按钮改由 `getFocusType` 门控；bulk 模拟条目仍不可进入近景（保持诚实标注）。

**50000 项质量优化第二轮已完成**：
- 修复 bulk 提升/回收的 `glowUniforms` 泄漏：`clearMaterializedBulkEntry` 回收时同步移除辉光 uniforms，反复点击星点不再永久增加每帧开销。
- 点击拾取只对可见对象 raycast：隐藏图层的 ~3.7 万点不再参与相交测试，点击成本约降 75%。
- 提升 bulk 目标时将底层批量星点颜色清零、回收时恢复（`setBulkPointVisible`），消除高细节对象下的重影星点。
- 关闭星图（toggle off）时同步回收已提升的 bulk 对象。
- `createLabel` 改为按文本估算宽度，去掉每标签一次的强制同步 reflow（重建时约 500 次）。
- 星图标记、深空核心与辉光壳改用共享单位球几何 + 缩放（`getSharedSphereGeometry`），共享几何在 `disposeObject3D` 中受保护；相关脉动动画改为基于 `userData.baseScale` 缩放。

**下一优先**：
1. 可选：木卫一/木卫二/土卫六等重点卫星个性化
2. 可选：太阳日冕纤维与黑子对比度继续微调
3. 可选：相机飞入曲线与信息面板布局打磨
4. 下一步如需继续扩容：建议先稳定 50000 项实机性能基线；迈向 10 万前应加入按天空分区加载、标签视锥裁剪和更高效的点选索引。

**实机性能基线**：
- 50000 项本地回归基线：三次页面重载约 `1.59–1.67 s`；Node 端目录解析约 `79–91 ms`，四层 bulk 几何缓冲合计 `1.188 MB`，四层切换、UGC 22000 / LHS 13625 搜索与详情打开正常，未发现控制台错误。
- 30000 项本地回归基线：三次完整重载约 `1.70–1.97 s`；Node 端目录解析约 `56–60 ms`，四层 bulk 几何缓冲合计 `708 KB`，四层切换、UGC 17000 / LHS 8625 搜索与详情打开正常，未发现控制台错误。
- 20000 项本地启动基线：DOMContentLoaded 约 `1.42 s`、load 约 `1.68 s`；初始 JavaScript 堆约 `64.8 MB`，四层切换与搜索后约 `68.0 MB`，四层 bulk 几何缓冲合计约 `468 KB`，未发现控制台错误。
...
| 场景 | 邻域 (12505) | 银河 (12518) | 本星系群 (12490) | 宇宙近邻 (12487) |
|------|:-----------:|:-----------:|:---------------:|:---------------:|
| 空闲 (无交互) | 待测 | 待测 | 待测 | 待测 |
| 拖拽旋转 | 待测 | 待测 | 待测 | 待测 |
| 搜索 + 选中 | 待测 | 待测 | 待测 | 待测 |
| 近景 (行星) | N/A | N/A | N/A | N/A |
| 近景 (特殊天体) | N/A | N/A | N/A | N/A |
...
| # | 优化项 | 预期收益 | 实施状态 |
|---|--------|---------|---------|
| 1 | 标签视锥裁剪 + 距离预筛 | 减少 40-60% 的 project() 调用 | 已实施 |
| 2 | 距离预筛 (4000 单位外跳过) | 减少远端 bulk 标签更新 | 已实施 |
| 3 | 标签更新节流 (40ms) | 控制 CPU 开销 | 已有 |
| 4 | Bulk 点 LOD (视锥外降采样) | 降低 GPU 顶点压力 | 待测后决定 |
| 5 | 标签碰撞检测 | 减少重叠标签 DOM 更新 | 已有 |
| 6 | 图层切换时预建索引 | 减少搜索延迟 | 待测后决定 |
| 7 | 移动端手势适配 | 扩大设备覆盖 | 待产品化时 |

### 测试方法

1. 打开浏览器 DevTools → Performance 面板
2. 加载项目后等待 5s 稳定
3. 录制 10s 交互（包含：旋转、缩放、切换图层、搜索目标、进入/退出近景）
4. 记录平均 FPS、JS Heap 大小、帧渲染耗时
5. 填入上表后提交