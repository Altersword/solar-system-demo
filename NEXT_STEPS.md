...
**星图阶段 C 与后续克制扩充已完成**：`CELESTIAL_CATALOG` 现为 **10000** 个目标
（邻域 2505 / 银河 2518 / 本星系群 2490 / 宇宙近邻 2487），并按图层提供尺度距离壳。
...
**万级目录渲染基础已完成**：
- 已有 9500 个扩容目标标记为批量渲染层；四个星图图层各用一个 THREE.Points 批次承载 2375 项，启动时不再为它们逐项创建球体、光晕和标签。
- 搜索或点击批量星点时，目标会按需临时提升为高细节对象；切换到其他目标或图层后会回收该临时对象。
- 重点手工目录仍保留原有网格、标签、特效、信息面板与展开系统交互；后续扩容应默认走批量层。

**万级目录第一轮优化已完成**：
- 9500 个批量条目统一声明为模拟压力测试数据，并在搜索结果与详情面板中明确提示。
- 批量星点方向改用黄金角低差异分布；严格校验 9500 个批量方向无重复，同时增加名称、坐标范围和数据来源校验。
...
4. 下一步如需继续扩容：建议记录 10000 项实机性能基线；迈向更大规模前应加入按天空分区加载、标签视锥裁剪和更高效的点选索引。
